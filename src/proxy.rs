...
///
/// `_tail` is the post-prefix path remainder from the router. P0 forwards the
/// original path+query unchanged; P1's `strip_prefix` will rewrite the path
/// here using `_tail` and `route.strip_prefix` — the seam is intentional.
pub async fn proxy(
    state: &AppState,
    route: &Route,
    _tail: &str,
