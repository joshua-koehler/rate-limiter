...
- **Deferred within cross-cutting:** `Content-Length` recompute for *transformed* bodies lands with P3 (P2 already recomputes it for the buffered retry body — client `Content-Length` is stripped and hyper reframes from the known length).
- **Not yet built (stub cleanly, never half-wire):** P3 transforms (request/response headers + body mapping/envelope). Their config blocks already parse + validate so a full config loads today.
