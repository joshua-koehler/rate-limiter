...
## Scope & priority
Build in this order, stopping when time runs out; a few features done cleanly beats many half-done.
...
- Std-lib HTTP + YAML parser only; no gateway/proxy framework.
...
- **Rate limit override:** route `rate_limit` fully replaces global (no merge). Over limit → **429 + `Retry-After`**.
